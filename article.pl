...
while($line)
{
	...
	{
		insert_article($vnum,$inum,$month,$year,$title,$fid,$page,$text,$authname,$authid);
		$title = "";
		$fid = "";
		$page = "";
		$page_end = "";
...
sub insert_article()
{
	my($vnum,$inum,$month,$year,$title,$fid,$page,$text,$authname,$authid) = @_;
	my($sth1);
	...
	$sth1=$dbh->prepare("insert into article values('$vnum','$inum','$month','$year','$title','$fid','$page','$text','$authname','$authid','')");
	$sth1->execute();
